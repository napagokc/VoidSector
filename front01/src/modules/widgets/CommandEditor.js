...
export class CommandEditorWidget extends React.Component {
	...
	proceed_data_message = () => {
		let res = [];
		for (let k in get_ships_state()) res.push(k);

		this.setState({ ships: res });
	};

	onSendCommand = (e) => {
		send_command(this.state.target, this.state.ship_id, this.state.command, this.state);
	};
		...
		if (typeof option_descr === typeof 0) {
			input = (
				<input
					type="number"
					onChange={(e) => {
						this.setState({ [key]: Number.parseFloat(e.target.value) });
					}}
				></input>
			);
	...
	get_description = (key) => {
		...
		return <div className="commandParamsSection">{result}</div>;
	};
	...
	get_command_panel = () => {
		...
		return (
			<div className="commandEditorUnit">
				<select
					onChange={(e) => {
						this.setState({ ship_id: e.target.value });
					}}
				>
					{ships_options}
				</select>
	...
	render() {
		return <div className="CommandEditorWidget">{this.get_command_panel()}</div>;
	}
}
