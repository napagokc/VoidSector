...
export class BodyEditor extends React.Component {
	...
	compare_selection_heavy = (body) => {
		for (let k in body) {
			if (!(k in this.state.selected_body_ref) || this.state.selected_body_ref[k] !== body[k]) return false;
		}
	...
	commit_attribute = () => {
		send_command('map_editor', 'admin', 'change_body', {
			descr: this.state.selected_body_edit,
			forced: this.state.forced
		});

		this.setState({ selected_body_idx: null });
	};

	//#endregion

	update_body_stats = () => {
		if (this.state.selected_body_edit.length === 0 || !('gr' in this.state.selected_body_edit)) return;

		return fetch(get_http_address() + '/utils/orbital_stats', {
			method: 'GET',
			headers: {
...
export class BodySpawner extends React.Component {
	...
	commit_spawn = () => {
		...
		this.setState({ spawned_entity_markId: '' });
	};
...
export class MapLoader extends React.Component {
	...
	componentWillUnmount() {
		clearInterval(timerscounter.get('MapLoader'));
	}

	updateContent = () => {
		return fetch(get_http_address() + '/admin/maps', {
			method: 'GET',
			headers: {}
		})
			.then((response) => {
	...
	render() {
		return (
			<div className="FileLoader">
				...
				<input
					onChange={(e) => {
						this.setState({ map_name: e.target.value });
					}}
					value={this.state.map_name}
				></input>
...
export class ShipScreenLoader extends React.Component {
	...
	componentWillUnmount() {
		clearInterval(timerscounter.get('ShipScreenLoader'));
	}

	updateContent = () => {
		return fetch(get_http_address() + '/admin/ship_screens', {
			method: 'GET',
			headers: {}
		})
			.then((response) => {
