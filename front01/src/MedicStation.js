...
export class MedicStation extends React.Component {
	...
	update_plague_matrix = () => {
		return fetch(get_http_address() + '/medicine/plague_matrix', {
			method: 'GET',
...
class PlayerRoleHealthCard extends React.Component {
	...
	apply_cure = (scale_name, cure_type) => {
		send_command('ship.med_sm', get_observer_id(), 'apply_' + cure_type + '_cure', {
	...
	add_points = (scale_name, points) => {
		send_command('ship.med_sm', get_observer_id(), 'add_points', {
			role: this.props.rolename,
			axis: scale_name,
			value: points
		});
	...
	apply_mutator = (mutator) => {
		send_command('ship.med_sm.plague', get_observer_id(), 'set_plague_v2_mutator', {
			role: this.props.rolename,
			mutator: mutator
		});
	};
			...
			>
				<label>
					...
					<input
						...
						max={6}
						min={1}
						value={this.state.selected_plague_phase_x}
						onChange={(e) => {
				...
				<label>
					...
					<input
						...
						max={6}
						min={1}
						value={this.state.selected_plague_phase_y}
						onChange={(e) => {
	...
	get_regulator = (scale_name) => {
		return (
			<div>
				<label>
					{scale_name}:{this.props.data[scale_name]} (
					{get_locales('health_state_' + scale_name + this.props.data[scale_name].toString())})
				</label>
	...
	render() {
		...
		return (
			<div className="PlayerRoleHealthCard">
				<label>
					<b>{get_locales(this.props.rolename)}</b>
				</label>
...
class PlagueMatrixSection extends React.Component {
	...
	is_cell_marked = (p1, p2, marked_cells) => {
		for (let i = 0; i < marked_cells.length; i++) {
			if (marked_cells[i][0] === p1 && marked_cells[i][1] === p2) return true;
		}
	...
	render() {
		return <div>{this.is_any_infected() ? this.get_plague_matrix() : <span></span>}</div>;
	}
}
